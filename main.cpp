...
const char kWindowTitle[] = "LE2B_13_サトウ_リュウセイ_MT3_02_04";
...
// 三角形描画
void DrawTriangle(const Triangle& t, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewPortMatrix);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	triangle.normal = { 1,1,1 };
	triangle.distance = 1.0f;
	...
	while (Novice::ProcessMessage() == 0) {
		...
		Novice::DrawLine(static_cast<int>(start.x), static_cast<int>(start.y), static_cast<int>(end.x), static_cast<int>(end.y), segment.color);
		///
		/// ↑描画処理ここまで
		///
...
bool IsCollision(const Triangle& t, const Segment& s) {
	Vector3 v01 = t.vertices[1] - t.vertices[0];
	Vector3 v02 = t.vertices[2] - t.vertices[0];

	Vector3 normal = Cross(v01, v02);
	normal = Normalize(normal);

	Plane plane{ normal, Dot(normal,t.vertices[0]) };
	...
	if (T >= 0.0f && T <= 1.0f) {
		...
		Vector3 intersection = s.origin + T * s.diff;

		for (int i = 0; i < 3; ++i) {

			Vector3 v = t.vertices[(i + 1) % 3] - t.vertices[i];

			Vector3 vp = intersection - t.vertices[i];
...
void DrawTriangle(const Triangle& t, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewPortMatrix) {
	Vector3 V[3] = {}; 
	V[0] = Transform(t.vertices[0], Multiply(viewProjectionMatrix, viewPortMatrix));
	V[1] = Transform(t.vertices[1], Multiply(viewProjectionMatrix, viewPortMatrix));
	V[2] = Transform(t.vertices[2], Multiply(viewProjectionMatrix, viewPortMatrix));
	Novice::DrawTriangle(int(V[0].x), int(V[0].y), int(V[1].x), int(V[1].y), int(V[2].x), int(V[2].y), 0xFFFFFFFF, kFillModeWireFrame);
}